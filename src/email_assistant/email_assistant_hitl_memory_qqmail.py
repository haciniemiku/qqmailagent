...
from email_assistant.tools.qqmail.prompt_templates import QQMAIL_TOOLS_PROMPT
from email_assistant.tools.qqmail.qqmail_tools import mark_as_read
...
from email_assistant.utils import llm, parse_qqmail, format_for_display, format_qqmail_markdown
from dotenv import load_dotenv

load_dotenv(".env")

# Get tools with QQ Mail tools
tools = get_tools(["send_email_tool", "schedule_meeting_tool", "check_calendar_tool", "Question", "Done"], include_qqmail=True)
...
def triage_router(state: State, store: BaseStore) -> Command[Literal["triage_interrupt_handler", "response_agent", "__end__"]]:
    ...
    author, to, subject, email_thread, email_id = parse_qqmail(state["email_input"])
    user_prompt = triage_user_prompt.format(
        author=author, to=to, subject=subject, email_thread=email_thread
    )

    # Create email markdown for Agent Inbox in case of notification  
    email_markdown = format_qqmail_markdown(subject, author, to, email_thread, email_id)
            ...
            llm_with_tools.invoke(
                [
                    {"role": "system", "content": agent_system_prompt_hitl_memory.format(
                        tools_prompt=QQMAIL_TOOLS_PROMPT,
                        background=default_background,
...
def interrupt_handler(state: State, store: BaseStore) -> Command[Literal["llm_call", "__end__"]]:
    ...
    for tool_call in state["messages"][-1].tool_calls:
        ...
        author, to, subject, email_thread, email_id = parse_qqmail(email_input)
        original_email_markdown = format_qqmail_markdown(subject, author, to, email_thread, email_id)
